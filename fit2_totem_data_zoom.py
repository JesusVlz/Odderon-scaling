...
from mpl_toolkits.axes_grid1.inset_locator import mark_inset  # Faltaba esta importación
...
# ============================================================
# FIT 2 Model (Pomeron Only - Energy Dependent Phase)
# ============================================================
def model_scaled_y_fit2(tss, sqrts_TeV, N10, B10, N20, B20, phi,
                        alpha=ALPHA, gamma_over_2=GAMMA_OVER_2, nu=NU):
    ...
    tan_factor = np.tan(nu * np.pi / 4.0)
    phi_P = phi + (B2 - B1) * tabs * np.power(s, gamma_over_2) * tan_factor

    A2 = (N1**2) * (e1**2) + (N2**2) * (e2**2) + 2.0 * N1 * N2 * e1 * e2 * np.cos(phi_P)
...
# ============================================================
def residuals_fit2(params, tss, sqrts, y, yerr):
    y_fit = model_scaled_y_fit2(tss, sqrts, *params)
    return (y - y_fit) / yerr
...
popt1, err1, chi2_1, ndof_1 = run_fit_scenario(residuals_fit2, model_scaled_y_fit2, tss_full, sqrts_full, y_full, yerr_full, n_starts, seed)
popt2, err2, chi2_2, ndof_2 = run_fit_scenario(residuals_fit2, model_scaled_y_fit2, tss_dp, sqrts_dp, y_dp, yerr_dp, n_starts, seed)
            
print_fit_results("FIT 2 (Full Domain)", popt1, err1, chi2_1, ndof_1)
print_fit_results("FIT 2 (Dip/Bump Region)", popt2, err2, chi2_2, ndof_2)

# ============================================================
# Plotting
# ============================================================
# CAMBIO IMPORTANTE: Crear explícitamente el objeto figura y ejes (fig, ax)
...
ygrid1 = model_scaled_y_fit2(xgrid, np.full_like(xgrid, Erep), *popt1)
ygrid2 = model_scaled_y_fit2(xgrid, np.full_like(xgrid, Erep), *popt2)
    
# GRAFICAR CURVAS EN EJE PRINCIPAL
ax.plot(xgrid, ygrid1, "-", color="orange", linewidth=2, label="FIT 2 (Full Domain)", zorder=10)
ax.plot(xgrid, ygrid2, "-", color="purple", linewidth=2, label="FIT 2 (Dip/Bump Region)", zorder=10)
...
# REPETIR LOS DATOS EN EL RECUADRO (Usando los datos reales de este script)
for i, E in enumerate(sorted(np.unique(sqrts_plot))):
    mE = np.isclose(sqrts_plot, E)
    axins.errorbar(
        tss_plot[mE], y_plot[mE], yerr=yerr_plot[mE],
...
axins.plot(xgrid, ygrid1, "-", color="orange", linewidth=2, zorder=10)
axins.plot(xgrid, ygrid2, "-", color="purple", linewidth=2, zorder=10)
...
outpath = os.path.join(outdir, "fit2_totem_scaled_data_zoom.pdf")
