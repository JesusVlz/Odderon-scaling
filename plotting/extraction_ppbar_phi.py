...
PARAMS_STD_b_phiplus = {'N10': 10.17, 'N20': 0.45, 'B10': 5.76, 'B20': 1.34, 'phi': 2.69,    'theta': -0.09}
...
med_1, low_1, high_1 = get_error_band(t, s, PARAMS_STD_b, PARAMS_PLUS_b, ERRORS_STD_b, ERRORS_PLUS_b)

med_2, low_2, high_2 = get_error_band(t, s, PARAMS_STD_b_phiplus, PARAMS_PLUS_b, ERRORS_STD_b, ERRORS_PLUS_b)
...
ax.plot(t, med_1, linestyle='-', color=colors[0], label=r"$p\bar{p}$ (Fit 1 $\phi -$)")
ax.fill_between(t, low_1, high_1, color=colors[0], alpha=0.2)

ax.plot(t, med_2, linestyle=':', color=colors[1], label=r"$p\bar{p}$ (Fit 1 $\phi +$)")  # This is the same as the first curve but with phi flipped, so we use a different linestyle to distinguish them
...
plt.savefig(os.path.join(outdir, "extraction_ppbar_phi_flip.pdf"), dpi=150, bbox_inches="tight")
